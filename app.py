...
def initialize_session_state():
    ...
    if 'json_data' not in st.session_state:
        st.session_state.json_data = {'lecture_names': [], 'records': {}}  # Initialize for guest
    if 'result_df' not in st.session_state:
        st.session_state.result_df = None
    if 'active_tab' not in st.session_state:
        st.session_state.active_tab = "SRT Parser"

def save_json_to_s3(user_id, file_path, data):
...
def load_json_from_s3(user_id, file_path):
    """Load JSON data from S3 for a user."""
    try:
        response = s3_client.get_object(Bucket=BUCKET_NAME, Key=f"{user_id}/{file_path}")
        ...
        with login_tab:
            ...
            with col1:
                if st.button("Login"):
                    ...
                    if user_id in credentials and credentials[user_id] == password:
                        ...
                        # Load lecture_names from S3
                        lecture_names = load_json_from_s3(user_id, "lecture_names.json") or []
                        st.session_state.json_data = {
                            'lecture_names': lecture_names,
                            'records': {}  # Will be loaded per lecture as needed
                        }
            ...
            with col2:
                if st.button("Guest Login"):
                    ...
                    st.session_state.is_authenticated = False
                    st.session_state.json_data = {'lecture_names': [], 'records': {}}
